...
def plot_training_history(loss, val_loss, acc, val_acc, mini_batch_scores, mini_batch_labels):
    ...
    plt.savefig('/share/home/biopharm/xuxiaobin/covid19/COVID_etpred/data/figure/loss/loss_fig.eps', dpi=350)
...
def plot_attention(weights):
    ...
    plt.savefig('/share/home/biopharm/xuxiaobin/covid19/COVID_etpred/data/figure/attention/weight.eps', dpi=350)
...
def train_rnn(model, verify, epochs, learning_rate, batch_size, X, Y, X_test, Y_test, show_attention,cell_type, subtype, position,model_path,
              save_path = '/share/home/biopharm/xuxiaobin/covid19/COVID_etpred/reports/figures_train/',
              out_path = '/share/home/biopharm/xuxiaobin/covid19/COVID_etpred/reports/pre_prob'):
    """
    ...
    for epoch in range(epochs):
        ...
        for count in range(0, num_of_examples - batch_size + 1, batch_size):
            ...
            # for IndexError: Target 1 is out of bounds
            # scores = model(X_batch) 

            scores, _ = model(X_batch, hidden)
            if cell_type == 'rnn1':
                score_raw = scores.float()
                scores = torch.abs(scores.flatten().float())
                loss = criterion(scores/torch.max(scores), Y_batch.float())
            else:
            ...
            if cell_type == 'rnn1':
                predictions = predictions_from_output(score_raw)
            else:
        ...
        with torch.no_grad():
            ...
            X_batch = X_test.contiguous()

            # test_scores, _ = model(X_test, model.init_hidden(Y_test.shape[0]).contiguous())
            test_scores, _ = model(X_test, model.init_hidden(Y_test.shape[0]))
            # pred_prob = test_scores
            # predictions = [1 if i>0.5 else 0 for i in pred_prob]
            ...
            pred_prob_out = pred_prob.cpu()
            pred_prob_out = pred_prob_out.numpy()
            predictions_out = predictions.cpu()
            predictions_out = predictions_out.numpy()

            pred_lst = []
            pred_prob_lst = []
            for pred in predictions_out:
                pred_lst.append(pred.item())
            for pred_probs in pred_prob_out:
                pred_prob_lst.append(pred_probs.item())

            for idex in range(len(pred_lst)):
                if pred_lst[idex] == 0:
                    pred_prob_lst[idex] = 1 - pred_prob_lst[idex]
                
            pred_prob_lst_all.append(sum(pred_prob_lst) / len(pred_prob_lst))


            precision, recall, fscore, mcc, val_acc = validation.evaluate(Y_test, predictions)
      
            # val_loss = criterion(test_scores, Y_test).item()
            val_loss = criterion(test_scores, Y_test)
            ...
            if val_acc>best_val_acc:
                ...
                # save the model
                save_file = os.path.join(model_path, cell_type+ f'_{position}_{subtype}.pt')
    ...
    pred_prob_lst_mean = sum(pred_prob_lst_all) / len(pred_prob_lst_all)
    print('predict probability = ' + str(pred_prob_lst_mean))
    ...
    # output the predict probability
    save_file = os.path.join(out_path, str(cell_type) + f'_{subtype}.csv')
    
    if not os.path.exists(save_file):
        with open(save_file, 'w', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(['site', 'predict probability'])

    with open(save_file, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([str(position), str(pred_prob_lst_mean)])
...
def preidict_rnn(model, verify, epochs, learning_rate, batch_size, X, Y, X_test, Y_test, show_attention,cell_type):
    """
    Training loop for a model utilizing hidden states.
