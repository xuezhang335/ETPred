...
class RnnModel(nn.Module):
    ...
    def __init__(self, num_layers, input_dim, output_dim, hidden_size, dropout_p, cell_type):
        ...
        if cell_type == 'LSTM':
            ...
            self.encoder = nn.GRU(input_dim, hidden_size, num_layers)
        elif cell_type == 'RNN':
            self.encoder = nn.RNN(input_dim, hidden_size, num_layers)
    ...
    def forward(self, input_seq, hidden_state):
        ...
        # move input_seq and hidden_state to gpu
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        input_seq = input_seq.to(device)
        hidden_state = input_seq.to(device)

        input_seq = input_seq.contiguous()
        hidden_state = hidden_state.contiguous()

        # Update the dimensions of hidden_state 
        # if lstm or da-rnn
        # hidden_state = hidden_state.unsqueeze(0)  # Add a new dimension at the beginning
    ...
    def init_hidden(self, batch_size):
        if self.cell_type == 'LSTM':
            h_init = torch.zeros(24, batch_size, self.hidden_size)
            c_init = torch.zeros(24, batch_size, self.hidden_size)

            return (h_init, c_init)
        elif self.cell_type == 'GRU':
            return torch.zeros(24, batch_size, self.hidden_size)
...
class AttentionModel(nn.Module):
    ...
    def forward(self, input_seq, hidden_state):
        ...
        self.score_seq = self.out(attn_applied.reshape(-1, self.hidden_size))

        return self.score_seq, attn_weights
